...
{
  console.log(`(!) Starting crash report extraction...`);
  fs.readdir(UPLOAD_DIR, (err, crashFiles) => {
    ...
    crashFiles.forEach(crashFile => {
      ...
      if (crashFileExt != ".ue4crash") return;
      ...
      zlib.inflate(fileBuffer, (err, buf) => {
        ...
        var DirectoryName = buf.slice(4, 264).toString("utf8").replace(/\0.*$/g,''); // 3 - 262 <-- FString: 260 bytes
        ...
        while (bytePtr < bufEnd)
        {
          ...
          bytePtr += 8;
          var FileName = buf.slice(bytePtr, bytePtr + 260).toString("utf8").replace(/\0.*$/g,'');  // 544 - 803 <-- FString: 260 bytes
          bytePtr += 260;
          ...
          fs.writeFileSync(`${reportDir}/${FileName}`, FileData);
